<template>
    <ul class="menu">
        <li class="item-talhao">
            ...
            <div class="text">
                <h3 class="text-1">{{discipline?.datas?.nome}}</h3>
                <h3 class="infos-accordion"> 
                  <span class="aulas">
                    Aulas: {{ discipline?.datas?.aulas?.quantidade }} | {{ discipline?.datas?.aulas?.horario }}
                  </span>
                  <span class="exercicios">
                    Exercícios: {{ discipline?.datas?.exercicios?.quantidade }} | {{ discipline?.datas?.exercicios?.horario }}
                  </span>
                  <span class="materias">
                    Matérias: {{ discipline?.datas?.materias?.quantidade }} | {{ discipline?.datas?.materias?.horario }}
                  </span>
                </h3>
            ...
            <div class="text-open" v-if="opened">
                <AccordionCompentence></AccordionCompentence>
            </div>
        </li>
    </ul>
</template>
...
<script setup>
  ...
  onMounted(() => {
      console.log("AD",discipline?.datas); 
  });
